...
{
    for (const auto func_name : s_functionNames)
    {
        const auto curr_function = functionNameMapping(func_name);
        auto selected = m_selectedFunctions.contains(curr_function);

        if (ImGui::Checkbox(func_name.data(), &selected))
        {
            if (selected)
                m_selectedFunctions.insert(curr_function);
            else
                m_selectedFunctions.erase(curr_function);
        }
    }
}
...
void AdvCalc::DrawPlot()
{
    ...
    if (m_selectedFunctions.size() == 0 ||
        m_selectedFunctions.size() == 1 &&
            *m_selectedFunctions.begin() == Function::NONE)
    {
