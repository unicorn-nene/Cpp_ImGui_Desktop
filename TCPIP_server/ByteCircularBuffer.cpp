 ...
 * @brief 向环形缓冲区写入数据
 *
 * @param bcb 指向环形缓冲区 ByteCircularBuffer_t 的指针
 * @param data 要写入的数据
 * @param data_size 写入数据的字节数
 * @return uint16_t 实际写入的数据的字节数
 */
 ...
 * @brief 从环形缓冲区中读取数据
 *
 * @param bcb 环形缓冲区 ByteCircularBuffer_t 指针
 * @param buffer 接受环形缓冲区数据的外部缓冲区
 * @param data_size 要读取的字节数
 * @param remove_read_bytes 是否从环形缓冲区中删除以读取的数据
 * @return uint16_t 实际读取的字节数目
 */
